...
class FPSModelCam : public Camera {
...
public:
	...
	FPSModelCam(); // initialise camera parameters so it is placed at the origin looking down the -z axis (for a right-handed camera) or +z axis (for a left-handed camera)

	FPSModelCam(float _theta, float _phi, float _radius, float _fovy, float _aspect, float _nearPlane, float _farPlane, glm::vec4 _pos);
	...
	void moveCamera(float deltaForward, float deltaRight, float deltaUp, float speed) override;
